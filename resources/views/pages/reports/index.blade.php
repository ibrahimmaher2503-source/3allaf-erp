@php
    ...
    $report['modules'] = array_values(array_diff($report['modules'], ['parties', 'assets']));
    $report['kpis'] = array_diff_key($report['kpis'], array_flip([
        'party_wallet_balance', 'party_booking_count', 'upcoming_party_count', 'party_invoice_count',
        'party_balance_due', 'assets_available', 'assets_reserved', 'asset_issues',
    ]));
    $report['detail_sections'] = collect($report['detail_sections'])->reject(fn ($section) => in_array($section['key'], [
        'party_wallet', 'party_bookings', 'party_invoices', 'party_payments', 'party_operations',
        'rental_assets', 'asset_reservations', 'asset_returns', 'asset_events',
    ], true))->values()->all();
    $report['visuals'] = collect($report['visuals'] ?? [])->reject(fn ($visual) => in_array($visual['key'], [
        'customer_party_wallet', 'party_pipeline', 'party_balance', 'asset_status',
    ], true))->values()->all();
    ...
    $moneyKeys = ['gross_sales', 'net_sales', 'total_sales', 'net_after_refunds', 'average_invoice_value', 'discounts', 'tax', 'payments_collected', 'refunds', 'stock_value', 'purchase_order_value', 'purchase_invoice_value', 'purchase_return_value', 'shift_variance', 'product_wallet_balance', 'gift_card_issued', 'gift_card_redeemed', 'gift_card_outstanding', 'quotation_value'];
    $integerKeys = ['invoice_count', 'purchase_order_count', 'purchase_invoice_count', 'purchase_return_count', 'open_shifts', 'unclosed_shifts', 'customer_count', 'new_customer_count', 'loyalty_earned', 'loyalty_redeemed', 'quotation_count'];
    $drilldownRoutes = ['gross_sales' => 'sales.index', 'net_sales' => 'sales.index', 'total_sales' => 'sales.index', 'net_after_refunds' => 'sales.index', 'invoice_count' => 'sales.index', 'average_invoice_value' => 'sales.index', 'discounts' => 'sales.index', 'tax' => 'sales.index', 'payments_collected' => 'sales.index', 'refunds' => 'sales.index', 'stock_on_hand' => 'inventory.index', 'stock_value' => 'inventory.index', 'quotation_count' => 'quotations.index', 'quotation_value' => 'quotations.index', 'purchase_order_count' => 'purchasing.orders', 'purchase_order_value' => 'purchasing.orders', 'purchase_invoice_count' => 'purchasing.invoices', 'purchase_invoice_value' => 'purchasing.invoices', 'purchase_return_count' => 'purchasing.returns', 'purchase_return_value' => 'purchasing.returns', 'open_shifts' => 'pos.shift', 'unclosed_shifts' => 'pos.shift', 'shift_variance' => 'pos.shift', 'customer_count' => 'customers.index', 'new_customer_count' => 'customers.index', 'loyalty_earned' => 'customers.index', 'loyalty_redeemed' => 'customers.index', 'product_wallet_balance' => 'customers.index', 'gift_card_outstanding' => 'gift.cards.index'];
    $meaningfulFilterKeys = collect($report['filters'])->filter(fn ($value, $key) => filled($value) && ! in_array($key, ['date_from', 'date_to', 'module'], true));
    $filterLabels = [
        'search' => __('Search'), 'branch_id' => __('Branch'), 'store_id' => __('Store'),
        'user_id' => __('User / cashier'), 'product_id' => __('Product'), 'category_id' => __('Category'),
        'product_type' => __('Product type'), 'product_status' => __('Product status'), 'brand_id' => __('Brand'),
        'age_label_id' => __('Age'), 'character_id' => __('Character'), 'colour_id' => __('Colour'),
        'gender_id' => __('Gender'), 'payment_method_id' => __('Payment method'), 'customer_id' => __('Customer'),
        'supplier_id' => __('Supplier'), 'document_status' => __('Document status'),
        'sort' => __('Sort'), 'direction' => __('Direction'),
    ];
    $filterValueMaps = [
        ...
        'payment_method_id' => $paymentMethods->mapWithKeys(fn ($item) => [$item->id => $item->code.' · '.$localizedName($item)]),
        'customer_id' => $customers->mapWithKeys(fn ($item) => [$item->id => $localizedName($item)]),
        'supplier_id' => $suppliers->mapWithKeys(fn ($item) => [$item->id => $item->code.' · '.$localizedName($item)]),
        'brand_id' => $brands->mapWithKeys(fn ($item) => [$item->id => $localizedName($item)]),
        'age_label_id' => $ages->mapWithKeys(fn ($item) => [$item->id => $localizedName($item)]),
        'character_id' => $characters->mapWithKeys(fn ($item) => [$item->id => $localizedName($item)]),
        'colour_id' => $colours->mapWithKeys(fn ($item) => [$item->id => $localizedName($item)]),
        'gender_id' => $genders->mapWithKeys(fn ($item) => [$item->id => $localizedName($item)]),
    ];
    $formatFilterValue = function (string $key, mixed $value) use ($filterValueMaps): string {
        if (isset($filterValueMaps[$key])) {
            return (string) ($filterValueMaps[$key][$value] ?? '#'.$value);
        }
        if (in_array($key, ['product_type', 'product_status', 'document_status', 'party_status', 'direction'], true)) {
...
<x-layouts::app :title="$reportTitle ?? __('Reports')">
    <x-app.page :title="$reportTitle ?? __('Dashboard & KPI reports')" :description="$reportMeta['description']" :eyebrow="$reportMeta['eyebrow']" max-width="7xl" data-report-dashboard>
        <x-slot:actions>
            <div class="flex flex-wrap items-center justify-end gap-2">
                @if (Gate::allows('dashboard_reports.export_xlsx') || Gate::allows('dashboard_reports.export_pdf'))
                    @foreach(['csv' => __('CSV'), 'xlsx' => __('Excel'), 'pdf' => __('PDF')] as $format => $label)
                        @can('dashboard_reports.export_'.($format === 'csv' ? 'xlsx' : $format))
                            <form method="POST" action="{{ route('reports.export') }}">@csrf @foreach($report['filters'] as $key => $value) @continue(in_array($key, ['page', 'per_page'], true)) <input type="hidden" name="{{ $key }}" value="{{ $value }}">@endforeach<input type="hidden" name="format" value="{{ $format }}"><flux:button type="submit" size="sm" :icon="$format === 'xlsx' ? 'table-cells' : 'document-arrow-down'" :aria-label="__('Create :format export', ['format' => $label])">{{ $label }}</flux:button></form>
                        @endcan
                    @endforeach
                @endif
        ...
        @if($reportKey === 'dashboard' && (Gate::allows('dashboard_reports.export_xlsx') || Gate::allows('dashboard_reports.export_pdf')))
        <section class="overflow-hidden rounded-2xl border border-border bg-surface shadow-card" aria-labelledby="dataset-export-heading">
            <header class="border-b border-border bg-surface-muted/40 px-4 py-3"><h2 id="dataset-export-heading" class="font-semibold text-text-primary">{{ __('Create a data export') }}</h2><p class="mt-1 text-sm text-text-muted">{{ __('Choose one dataset and the exact operating scope. The generated file stays private to your account.') }}</p></header>
            <form method="POST" action="{{ route('reports.export') }}" class="grid gap-3 p-4 sm:grid-cols-2 lg:grid-cols-4">@csrf
            <flux:select name="dataset" :label="__('Export dataset')" required>
                @foreach($datasetLabels as $dataset => $label)<option value="{{ $dataset }}">{{ $label }}</option>@endforeach
            </flux:select>
            ...
            <div class="flex items-end"><flux:button type="submit" variant="primary" icon="document-arrow-down" wire:loading.attr="disabled">{{ __('Request export') }}</flux:button></div>
            </form>
        </section>
        @endif

        <nav class="flex gap-2 overflow-x-auto rounded-xl border border-border bg-surface p-2" aria-label="{{ __('Report categories') }}">
            ...
            @endforeach
            @can('pos_sales.view') @can('pos_sales.payment_view')
                <a href="{{ route('reports.sales-summary') }}" class="inline-flex min-h-10 shrink-0 items-center gap-2 rounded-xl border border-border bg-surface px-3 text-sm font-semibold text-text-muted transition hover:border-primary/40 hover:text-text-primary"><flux:icon name="presentation-chart-line" class="size-4" />{{ __('Sales Summary') }}</a>
                <a href="{{ route('reports.sales-by-product') }}" class="inline-flex min-h-10 shrink-0 items-center gap-2 rounded-xl border border-border bg-surface px-3 text-sm font-semibold text-text-muted transition hover:border-primary/40 hover:text-text-primary"><flux:icon name="cube" class="size-4" />{{ __('Sales by Product') }}</a>
            @endcan @endcan
        </nav>
        ...
        <section class="overflow-hidden rounded-2xl border border-border/80 bg-surface shadow-card" aria-labelledby="report-filters-heading">
            <div class="flex flex-col gap-2 border-b border-border/70 bg-surface-muted/40 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex flex-wrap gap-2 border-b border-border/70 px-4 py-2" aria-label="{{ __('Date presets') }}"><span class="me-1 self-center text-xs font-semibold text-text-muted">{{ __('Date presets') }}</span><a class="rounded-lg border border-border px-3 py-2 text-xs font-semibold hover:border-primary/40" href="{{ request()->fullUrlWithQuery(['date_from' => now()->toDateString(), 'date_to' => now()->toDateString(), 'page' => 1]) }}">{{ __('Today') }}</a><a class="rounded-lg border border-border px-3 py-2 text-xs font-semibold hover:border-primary/40" href="{{ request()->fullUrlWithQuery(['date_from' => now()->subDays(6)->toDateString(), 'date_to' => now()->toDateString(), 'page' => 1]) }}">{{ __('Last 7 days') }}</a><a class="rounded-lg border border-border px-3 py-2 text-xs font-semibold hover:border-primary/40" href="{{ request()->fullUrlWithQuery(['date_from' => now()->subDays(29)->toDateString(), 'date_to' => now()->toDateString(), 'page' => 1]) }}">{{ __('Last 30 days') }}</a></div>
            @if($meaningfulFilterKeys->isNotEmpty())<div class="flex flex-wrap gap-2 border-b border-border/70 px-4 py-2" aria-label="{{ __('Active filters') }}">@foreach($meaningfulFilterKeys as $key => $value)<a href="{{ request()->fullUrlWithQuery([$key => null, 'page' => 1]) }}" class="inline-flex min-h-9 items-center gap-1 rounded-full bg-primary-soft px-3 text-xs font-semibold text-primary" aria-label="{{ __('Remove filter: :filter', ['filter' => $filterLabels[$key] ?? __('Filter')]) }}"><span>{{ $filterLabels[$key] ?? __('Filter') }}: {{ $formatFilterValue($key, $value) }}</span><flux:icon name="x-mark" class="size-3.5" /></a>@endforeach</div>@endif
            <form method="GET" action="{{ url()->current() }}" class="grid gap-3 p-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                ...
                @if($showFor(['inventory']))
                    <flux:select name="product_type" label="{{ __('Product type') }}"><option value="">{{ __('All product types') }}</option>@foreach(['standard','service','bundle'] as $type)<option value="{{ $type }}" @selected($report['filters']['product_type'] === $type)>{{ __((string) str($type)->replace('_',' ')->title()) }}</option>@endforeach</flux:select>
                    <flux:select name="product_status" label="{{ __('Product status') }}"><option value="">{{ __('All product statuses') }}</option>@foreach(['active','inactive','archived'] as $status)<option value="{{ $status }}" @selected($report['filters']['product_status'] === $status)>{{ __((string) str($status)->title()) }}</option>@endforeach</flux:select>
                    @foreach([['brand_id',$brands,'Brand','All brands'],['age_label_id',$ages,'Age','All ages'],['character_id',$characters,'Character','All characters'],['colour_id',$colours,'Colour','All colours'],['gender_id',$genders,'Gender','All genders']] as [$field,$options,$label,$empty])
                        @if($options->isNotEmpty())<flux:select name="{{ $field }}" label="{{ __($label) }}"><option value="">{{ __($empty) }}</option>@foreach($options as $option)<option value="{{ $option->id }}" @selected((string) $report['filters'][$field] === (string) $option->id)>{{ str_starts_with(app()->getLocale(), 'ar') ? ($option->name_ar ?: $option->name_en) : ($option->name_en ?: $option->name_ar) }}</option>@endforeach</flux:select>@endif
                    @endforeach
                @endif
                @if($showFor(['sales', 'cash']) && $paymentMethods->isNotEmpty())<flux:select name="payment_method_id" label="{{ __('Payment method') }}"><option value="">{{ __('All payment methods') }}</option>@foreach($paymentMethods as $method)<option value="{{ $method->id }}" @selected((string) $report['filters']['payment_method_id'] === (string) $method->id)>{{ $method->code }} · {{ str_starts_with(app()->getLocale(), 'ar') ? ($method->name_ar ?: $method->name_en) : ($method->name_en ?: $method->name_ar) }}</option>@endforeach</flux:select>@endif
                @if($showFor(['sales', 'customers']) && $customers->isNotEmpty())<flux:select name="customer_id" label="{{ __('Customer') }}"><option value="">{{ __('All customers') }}</option>@foreach($customers as $customer)<option value="{{ $customer->id }}" @selected((string) $report['filters']['customer_id'] === (string) $customer->id)>{{ str_starts_with(app()->getLocale(), 'ar') ? ($customer->name_ar ?: $customer->name_en) : ($customer->name_en ?: $customer->name_ar) }}@if(isset($customer->phone_display)) · {{ $customer->phone_display }}@endif</option>@endforeach</flux:select>@endif
                @if($showFor(['purchasing']) && $suppliers->isNotEmpty())<flux:select name="supplier_id" label="{{ __('Supplier') }}"><option value="">{{ __('All suppliers') }}</option>@foreach($suppliers as $supplier)<option value="{{ $supplier->id }}" @selected((string) $report['filters']['supplier_id'] === (string) $supplier->id)>{{ $supplier->code }} · {{ str_starts_with(app()->getLocale(), 'ar') ? ($supplier->name_ar ?: $supplier->name_en) : ($supplier->name_en ?: $supplier->name_ar) }}</option>@endforeach</flux:select>@endif
        ...
        <x-cards.section-card :title="__('Source reconciliation')" :description="__('Every displayed total comes from the authoritative source records under this exact filter set.')">
            ...
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                @if(in_array('sales', $report['modules'], true))<div class="rounded-xl border border-border bg-surface-muted/30 p-4"><p class="text-xs font-semibold uppercase tracking-wide text-text-muted">{{ __($report['sources']['sales_label']) }}</p><p class="mt-2 text-xl font-bold tabular-nums text-text-primary">{{ number_format($report['sources']['sales_count']) }}</p><p class="mt-1 text-xs text-text-muted">{{ number_format($report['sources']['sales_total'], 2) }} {{ __('final value') }}</p></div>@endif
            ...
            <div class="space-y-5">
                @foreach($report['detail_sections'] as $section)
                    <x-tables.data-panel :title="$section['title']" :description="trans_choice(':count row in the selected scope|:count rows in the selected scope', count($section['rows']), ['count' => count($section['rows'])])">
                        ...
                        <table class="data-table min-w-[720px] w-full text-sm"><thead><tr>@foreach($section['columns'] as $key => $label)<th>@if(isset($section['sortable'][$key]))<a href="{{ request()->fullUrlWithQuery(['sort' => $key, 'direction' => (($report['filters']['sort'] ?? null) === $key && ($report['filters']['direction'] ?? 'desc') === 'asc') ? 'desc' : 'asc', 'page' => 1]) }}" class="underline decoration-dotted underline-offset-4">{{ $label }}@if(($report['filters']['sort'] ?? null) === $key) {{ ($report['filters']['direction'] ?? 'desc') === 'asc' ? '↑' : '↓' }}@endif</a>@else{{ $label }}@endif</th>@endforeach</tr></thead><tbody>
                        ...
                        @if(!empty($section['pagination']))<nav class="mt-4 flex flex-col gap-3 text-sm text-text-muted sm:flex-row sm:items-center sm:justify-between"><span>{{ trans_choice(':count row|:count rows', $section['pagination']['total'], ['count' => $section['pagination']['total']]) }} · {{ __('Page :current of :last', ['current' => $section['pagination']['current_page'], 'last' => $section['pagination']['last_page']]) }}</span><div class="flex gap-2">@if($section['pagination']['current_page'] > 1)<flux:button size="sm" variant="subtle" href="{{ request()->fullUrlWithQuery(['page' => $section['pagination']['current_page'] - 1]) }}">{{ __('Previous') }}</flux:button>@endif @if($section['pagination']['current_page'] < $section['pagination']['last_page'])<flux:button size="sm" variant="subtle" href="{{ request()->fullUrlWithQuery(['page' => $section['pagination']['current_page'] + 1]) }}">{{ __('Next') }}</flux:button>@endif</div></nav>@endif
                    </x-tables.data-panel>
                @endforeach
            </div>
        </section>
