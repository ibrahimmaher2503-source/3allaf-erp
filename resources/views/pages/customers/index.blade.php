<x-layouts::app :title="__('Customers')">
    ...
    <div class="mx-auto w-full max-w-7xl space-y-6 p-4 sm:p-6">
        ...
        <section data-guide="customer-search" class="internal-toolbar" aria-labelledby="customer-search-heading">
            <form method="GET" action="{{ route('customers.index') }}" class="grid gap-3 sm:grid-cols-2 lg:grid-cols-5 lg:items-end">
        ...
        <x-tables.data-panel data-guide="customer-table" :title="__('Profiles')" :description="__('Customer records in your authorized scope.')">
            ...
            <table class="data-table responsive-resource-table min-w-full text-start text-sm">
                    <thead><tr><th scope="col">{{ __('Customer') }}</th><th scope="col">{{ __('Customer group') }}</th><th scope="col">{{ __('Phone') }}</th><th scope="col">{{ __('Residence') }}</th><th scope="col">{{ __('Status') }}</th><th scope="col" class="text-end">{{ __('Action') }}</th></tr></thead>
                    <tbody>
                        @forelse ($customers as $customer)
                            <tr data-customer-row>
                                ...
                                <td data-label="{{ __('Residence') }}">{{ str_starts_with(app()->getLocale(), 'ar') ? $customer->governorate?->name_ar : $customer->governorate?->name_en }}@if($customer->city)<span class="block text-xs text-text-muted">{{ str_starts_with(app()->getLocale(), 'ar') ? $customer->city->name_ar : $customer->city->name_en }}</span>@elseif(!$customer->governorate)<span class="text-xs text-text-muted">{{ __('Legacy record — residence not recorded') }}</span>@endif</td>
                                ...
                                <td data-label="{{ __('Action') }}" class="text-end">
                                    ...
                                    @else
                                        <x-actions.button semantic="view" :label="__('Open profile')" :href="route('customers.show', $customer)">{{ __('Open profile') }}</x-actions.button>
                                    @endif
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="6"><x-state.empty :title="__('No customer profiles found.')" :description="__('Create a profile or broaden the search.')"><x-slot:action>@if ($mode === 'loyalty') @can('company_settings.view')<flux:button href="{{ route('admin.settings.customer-loyalty') }}" variant="subtle">{{ __('Review customer policy') }}</flux:button>@endcan @else @can('customers.create')<flux:button href="{{ route('customers.create') }}" variant="primary" icon="plus">{{ __('Create customer profile') }}</flux:button>@endcan @endif</x-slot:action></x-state.empty></td></tr>
                        @endforelse
                    </tbody>
                </table>
