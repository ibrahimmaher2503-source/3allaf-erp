...
final class DemoFeedStoreSeeder extends Seeder
{
    ...
    private function receiptsAndPayments(array $saleIds, array $purchaseIds, int $paymentMethodId, int $cashAccountId, int $userId, $now): void
    {
        ...
        $receiptId = DB::table('customer_receipts')->insertGetId(['public_id' => (string) Str::uuid(), 'customer_id' => $sale->customer_id, 'payment_method_id' => $paymentMethodId, 'cash_account_id' => $cashAccountId, 'receipt_date' => now()->subDays(5)->toDateString(), 'amount' => '500', 'reference' => 'SYN-RECEIPT-1', 'status' => 'approved', 'created_by' => $userId, 'approved_by' => $userId, 'approved_at' => $now, 'idempotency_key' => 'feed-receipt-1', 'payload_hash' => hash('sha256', 'feed-receipt-1'), 'created_at' => $now, 'updated_at' => $now]);
