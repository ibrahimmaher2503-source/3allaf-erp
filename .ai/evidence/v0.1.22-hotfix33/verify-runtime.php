...
$mode = $argv[1] ?? '--fixture';
$node = getenv('HOTFIX33_NODE') ?: '/opt/cpanel/ea-nodejs20/bin/node';
...
try {
    ...
    $assert(ApplicationVersion::RELEASE === '0.1.22-hotfix33', 'Hotfix33 version contract is not active.');
    ...
    $translator->addLines(['hotfix33.ar_probe' => 'AR_FALLBACK'], 'ar');
    $translator->addLines(['hotfix33.en_probe' => 'EN_FALLBACK'], 'en');
    $app->setLocale('ar-EG');
    $assert(__('hotfix33.ar_probe') === 'AR_FALLBACK', 'ar-EG did not fall back to ar.');
    $assert(__('hotfix33.en_probe') === 'EN_FALLBACK', 'ar-EG did not fall back through ar to en.');
    ...
    config(['session.driver' => 'array', 'session.lottery' => [0, 100]]);
    app('session')->forgetDrivers();
    $session = app('session')->driver();
    $session->start();
    $request = Request::create('/locale', 'POST', ['locale' => 'ar-EG']);
    $request->setLaravelSession($session);
    $app->instance('request', $request);
    $response = ($localeRoute->getAction('uses'))($request);
    $assert($response->getStatusCode() === 302, 'Locale switch did not return its expected redirect.');
    $assert($session->get('locale') === 'ar-EG', 'Locale route did not persist ar-EG in session.');
    $localeCookie = collect($response->headers->getCookies())->first(fn ($cookie) => $cookie->getName() === 'locale');
    $assert($localeCookie?->getValue() === 'ar-EG', 'Locale route did not persist ar-EG in cookie.');
    $middleware = new SetLocale();
    foreach (['ar', 'en', 'ar-EG'] as $preference) {
        $probeSession = new SessionStore('hotfix33-'.$preference, new ArraySessionHandler(120));
        $probeSession->start();
    ...
    $app->setLocale('ar-EG');
    $shell = view('layouts.auth.simple', ['slot' => new Illuminate\Support\HtmlString('<div data-hotfix33>تمام</div>')])->render();
    ...
    $runBrowser = static function (string $renderedLookup, string $supplierId) use ($assert, $jsAsset, $node, $root): void {
        ...
        $htmlFile = tempnam(sys_get_temp_dir(), 'hotfix33-browser-');
    ...
    echo "HOTFIX33_STATIC_LOCALE_SHELL_ASSET=PASS\n";
    if ($mode === '--fixture') {
        $runBrowser($lookup, '7');
        echo "HOTFIX33_LOCALE_SWITCH_SESSION_COOKIE_FALLBACK=PASS\n";
        echo "HOTFIX33_RUNTIME_MODE=FIXTURE_ONLY\n";
        echo "HOTFIX33_RUNTIME_VERIFICATION=PASS mode=fixture\n";
        exit(0);
    }
    ...
    try {
        ...
        foreach (User::query()->where('status', 'active')->whereNotNull('email_verified_at')->orderByDesc('is_super_admin')->orderBy('id')->limit(100)->get() as $candidate) {
            ...
            $required = ['dashboard_reports.view', 'products_categories_brands.view', 'purchase_orders.view', 'purchase_orders.create', 'purchase_invoices_supplier_returns.view', 'pricing_lists.view', 'pos_sales.view'];
        ...
        $screens = ['/dashboard', '/catalog/products', '/purchasing/orders', '/purchasing/invoices', '/pricing/lists/manage', '/pos', '/reports'];
        ...
        echo "HOTFIX33_AUTHENTICATED_AR_EN_AR_EG_SCREENS=PASS count=21 status=200\n";
        ...
        echo "HOTFIX33_INVOICE_SEARCH_KEYBOARD_RUNTIME=PASS\n";
    } finally {
        while ($connection->transactionLevel() > $level) {
            $connection->rollBack();
        }
    }
    echo "HOTFIX33_DATABASE_MUTATION=NONE transaction=rolled_back\n";
    echo "HOTFIX33_RUNTIME_VERIFICATION=PASS mode=activation\n";
} catch (Throwable $exception) {
    fwrite(STDERR, 'HOTFIX33_RUNTIME_VERIFICATION=FAIL '.str_replace(["\r", "\n"], ' ', $exception->getMessage())."\n");
    exit(1);
}
