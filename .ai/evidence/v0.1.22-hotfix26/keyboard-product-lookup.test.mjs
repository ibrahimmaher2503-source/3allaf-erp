...
const lookupFixture = () => {
    ...
    const root = element({
        dataset: {
            ...
            messageEmpty: 'empty', messageSupplierRequired: 'supplier required', messageSupplierEmpty: 'supplier empty',
...
test('incremental requests are deduplicated and stale responses cannot replace narrowed results', async () => {
    ...
    lookup.render = () => {};
    input.value = 'Rob';
    const broadA = lookup.search('Rob');
    const broadB = lookup.search('Rob');
    assert.equal(fetches, 1);
    input.value = 'Robot Alpha';
    const narrow = lookup.search('Robot Alpha');
...
test('exact barcode and keyboard selection prevent parent-form submission', async () => {
    const { lookup, input, results } = lookupFixture();
    const selected = [];
    lookup.select = (item) => selected.push(item.id);
    globalThis.fetch = async () => ({ ok: true, json: async () => ({ data: [{ id: 9, exact: true, item_code: 'SKU-9', barcodes: ['6220000000099'] }] }) });
