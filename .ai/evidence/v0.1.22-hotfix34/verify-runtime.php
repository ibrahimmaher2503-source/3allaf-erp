...
$mode = $argv[1] ?? '--fixture';
$node = getenv('HOTFIX34_NODE') ?: '/opt/cpanel/ea-nodejs20/bin/node';
...
try {
    ...
    $assert(ApplicationVersion::RELEASE === '0.1.22-hotfix34', 'Hotfix34 version contract is not active.');
    ...
    $translator->addLines(['hotfix34.ar_probe' => 'AR_FALLBACK'], 'ar');
    $translator->addLines(['hotfix34.en_probe' => 'EN_FALLBACK'], 'en');
    $app->setLocale('ar-EG');
    $assert(__('hotfix34.ar_probe') === 'AR_FALLBACK', 'ar-EG did not fall back to ar.');
    $assert(__('hotfix34.en_probe') === 'EN_FALLBACK', 'ar-EG did not fall back through ar to en.');
    ...
    foreach (['ar', 'en', 'ar-EG'] as $preference) {
        $probeSession = new SessionStore('hotfix34-'.$preference, new ArraySessionHandler(120));
        $probeSession->start();
    ...
    $app->setLocale('ar-EG');
    $shell = view('layouts.auth.simple', ['slot' => new Illuminate\Support\HtmlString('<div data-hotfix34>تمام</div>')])->render();
    ...
    $runBrowser = static function (string $renderedLookup, string $supplierId) use ($assert, $jsAsset, $node, $root): void {
        ...
        $htmlFile = tempnam(sys_get_temp_dir(), 'hotfix34-browser-');
    ...
    echo "HOTFIX34_ROUTE_CACHE_BOOT=PASS controller=invokable\n";
    echo "HOTFIX34_STATIC_LOCALE_SHELL_ASSET=PASS\n";
    if ($mode === '--fixture') {
        $runBrowser($lookup, '7');
        echo "HOTFIX34_LOCALE_SWITCH_SESSION_COOKIE_FALLBACK=PASS locales=ar,en,ar-EG invalid=rejected\n";
        echo "HOTFIX34_RUNTIME_MODE=FIXTURE_ONLY\n";
        echo "HOTFIX34_RUNTIME_VERIFICATION=PASS mode=fixture\n";
        exit(0);
    }
    ...
    try {
        ...
        echo "HOTFIX34_AUTHENTICATED_AR_EN_AR_EG_SCREENS=PASS count=21 status=200\n";
        ...
        echo "HOTFIX34_INVOICE_SEARCH_KEYBOARD_RUNTIME=PASS\n";
    } finally {
        while ($connection->transactionLevel() > $level) {
            $connection->rollBack();
        }
    }
    echo "HOTFIX34_DATABASE_MUTATION=NONE transaction=rolled_back\n";
    echo "HOTFIX34_RUNTIME_VERIFICATION=PASS mode=activation\n";
} catch (Throwable $exception) {
    fwrite(STDERR, 'HOTFIX34_RUNTIME_VERIFICATION=FAIL '.str_replace(["\r", "\n"], ' ', $exception->getMessage())."\n");
    exit(1);
}
