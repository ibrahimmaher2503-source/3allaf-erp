...
try {
    ...
    $assert(ApplicationVersion::RELEASE === '0.1.22-hotfix37', 'Candidate version mismatch.');
    ...
    try {
        $checks = [
            'ar' => ['/admin/branches', 'فتح مساعدة الفروع'],
            'en' => ['/admin/settings?tab=company', 'Show search and filters by default'],
            'ar-EG' => ['/admin/authorization-baseline', 'افتح مساعدة صلاحيات المستخدمين'],
        ];
        foreach ($checks as $locale => [$uri, $needle]) {
            ...
            $assert(str_contains($html, $needle), "{$locale} contextual content is missing.");
            $assert(str_contains($html, 'context-help__trigger'), "{$locale} help control is missing.");
            $assert(str_contains($html, 'x-trap.inert.noscroll'), "{$locale} focus trap is missing.");
            echo "HOTFIX37_AUTHENTICATED_SMOKE=PASS locale={$locale} uri={$uri}\n";
        }
    } finally {
        DB::rollBack();
    }

    echo "HOTFIX37_SMOKE_RENDER=PASS locales=ar,en,ar-EG database_mutation=none\n";
    exit(0);
} catch (Throwable $exception) {
    fwrite(STDERR, 'HOTFIX37_SMOKE_RENDER=FAIL '.$exception->getMessage()."\n");
    exit(1);
}
