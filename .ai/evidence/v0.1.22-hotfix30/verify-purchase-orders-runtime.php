...
$mode = $argv[1] ?? '--fixture';
$nodeBin = '/opt/cpanel/ea-nodejs20/bin/node';
...
$assert(ApplicationVersion::RELEASE === '0.1.22-hotfix30', 'Hotfix30 version contract is not active.');
...
foreach (['en' => ['ltr', 'text-left'], 'ar' => ['rtl', 'text-right']] as $locale => [$direction, $alignment]) {
    ...
    foreach (["dir=\"{$direction}\"", $alignment, 'data-product-search', 'data-camera-trigger', 'role="listbox"', 'aria-live="polite"', 'data-message-error=', 'flex items-stretch gap-1.5'] as $needle) {
...
foreach (['supplier_only', 'BarcodeDetector', 'exact_unique', 'productLookupReady', 'requestSequence', 'productResultsOverlay', 'same-origin', 'morph.updated', 'livewire:initialized'] as $needle) {
    $assert(str_contains($compiled, $needle), "Compiled product lookup omitted {$needle}.");
}

echo "HOTFIX30_STATIC_ROUTE_LAYOUT_ASSET=PASS\n";
if ($mode === '--fixture') {
    $fixtureOutput = getenv('HOTFIX_BROWSER_HTML_OUTPUT');
    if (is_string($fixtureOutput) && $fixtureOutput !== '') {
        $fixtureHtml = '<main data-guide="po-create-page"><form data-product-line-editor><div class="max-h-[32rem] overflow-y-auto" data-order-lines-scroll><div data-product-line>'.$lookup.'<input data-line-quantity value="2"><input data-line-price data-price-kind="cost"><p data-price-source></p><output data-line-total></output></div></div></form></main>';
        $assert(file_put_contents($fixtureOutput, $fixtureHtml) !== false, 'Could not write the rendered browser fixture.');
        echo "HOTFIX30_RENDERED_LOOKUP_FIXTURE=PASS\n";
    }
    echo "HOTFIX30_RUNTIME_MODE=FIXTURE_ONLY\n";
    exit(0);
}
...
try {
    ...
    foreach (['en', 'ar'] as $locale) {
        ...
        echo 'HOTFIX30_AUTHENTICATED_PURCHASE_ORDERS_'.strtoupper($locale)."=PASS status=200 store={$runtimeStore->id}\n";
        echo 'HOTFIX30_AUTHENTICATED_PURCHASE_ORDER_CREATE_'.strtoupper($locale)."=PASS status=200 store={$runtimeStore->id}\n";
    }
    ...
    echo "HOTFIX30_AUTHENTICATED_PRODUCT_SEARCH=PASS status=200 query_length=".mb_strlen($query)." matches={$matches->count()}\n";

    $assert(is_executable($nodeBin), 'Required Node 20 runtime is unavailable for the compiled browser contract.');
    $browserHtml = tempnam(sys_get_temp_dir(), 'hotfix30-browser-');
    $assert(is_string($browserHtml), 'Could not allocate the browser-contract HTML file.');
    try {
        $assert(file_put_contents($browserHtml, $create) !== false, 'Could not write the authenticated create-page HTML contract.');
        $browserOutput = [];
        $browserStatus = 1;
        exec(escapeshellarg($nodeBin).' '.escapeshellarg($root.'/.ai/evidence/v0.1.22-hotfix30/browser-search-contract.mjs').' '.escapeshellarg($root.'/public/build/'.$asset).' '.escapeshellarg($browserHtml).' '.escapeshellarg((string) $association->supplier_id).' 2>&1', $browserOutput, $browserStatus);
        $browserResult = implode("\n", $browserOutput);
        $assert($browserStatus === 0, "Compiled browser-search contract failed: {$browserResult}");
        foreach (['HOTFIX30_COMPILED_BROWSER_INITIAL_LOAD=PASS', 'HOTFIX30_COMPILED_BROWSER_ARABIC_AND_IDENTIFIER_INPUT=PASS', 'HOTFIX30_LOADING_RESULTS_EMPTY_ERROR_STATES=PASS', 'HOTFIX30_MULTI_RESULT_KEYBOARD_ROW_STATE=PASS', 'HOTFIX30_LIVEWIRE_NAVIGATION_MORPH_ADDED_ROW=PASS', 'HOTFIX30_DROPDOWN_VIEWPORT_OVERLAY_NOT_CLIPPED=PASS'] as $proof) {
            $assert(str_contains($browserResult, $proof), "Compiled browser contract omitted {$proof}.");
        }
        echo $browserResult."\n";
    } finally {
        if (is_file($browserHtml)) {
            unlink($browserHtml);
        }
    }
} finally {
...
echo "HOTFIX30_DATABASE_MUTATION=NONE transaction=rolled_back\n";
