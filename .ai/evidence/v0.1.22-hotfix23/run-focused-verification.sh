...
readonly EVIDENCE='.ai/evidence/v0.1.22-hotfix23'
readonly COMPILED_VIEWS="$PWD/storage/framework/views"

if [[ "${1:-}" != '--resume-after-layout' && "${1:-}" != '--resume-after-layout-contract' ]]; then
    "$PHP_BIN" "$EVIDENCE/verify-layout-runtime.php" --fixture
fi
if [[ "${1:-}" != '--resume-after-layout-contract' ]]; then
    "$NODE_BIN" "$EVIDENCE/verify-layout-contract.mjs"
fi
...
for file in \
    ...
    "$EVIDENCE/verify-layout-runtime.php"; do
...
[[ "$("$PHP_BIN" -r 'require "vendor/autoload.php"; echo App\Support\ApplicationVersion::RELEASE;')" == '0.1.22-hotfix23' ]]
echo 'HOTFIX23_FOCUSED_VERIFICATION=PASS visual=unavailable deterministic_layout=pass'
