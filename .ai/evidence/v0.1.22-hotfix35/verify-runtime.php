...
$mode = $argv[1] ?? '--fixture';
$node = getenv('HOTFIX35_NODE') ?: '/opt/cpanel/ea-nodejs20/bin/node';
...
try {
    ...
    $assert(ApplicationVersion::RELEASE === '0.1.22-hotfix35', 'Hotfix35 version contract is not active.');
    ...
    $translator->addLines(['hotfix35.ar_probe' => 'AR_FALLBACK'], 'ar');
    $translator->addLines(['hotfix35.en_probe' => 'EN_FALLBACK'], 'en');
    $app->setLocale('ar-EG');
    $assert(__('hotfix35.ar_probe') === 'AR_FALLBACK', 'ar-EG did not fall back to ar.');
    $assert(__('hotfix35.en_probe') === 'EN_FALLBACK', 'ar-EG did not fall back through ar to en.');
    ...
    foreach (['ar', 'en', 'ar-EG'] as $preference) {
        $probeSession = new SessionStore('hotfix35-'.$preference, new ArraySessionHandler(120));
        $probeSession->start();
    ...
    $app->setLocale('ar-EG');
    $shell = view('layouts.auth.simple', ['slot' => new Illuminate\Support\HtmlString('<div data-hotfix35>تمام</div>')])->render();
    ...
    $runBrowser = static function (string $renderedLookup, string $supplierId) use ($assert, $jsAsset, $node, $root): void {
        ...
        $htmlFile = tempnam(sys_get_temp_dir(), 'hotfix35-browser-');
    ...
    echo "HOTFIX35_ROUTE_CACHE_BOOT=PASS controller=invokable\n";
    echo "HOTFIX35_STATIC_LOCALE_SHELL_ASSET=PASS\n";
    if ($mode === '--fixture') {
        $runBrowser($lookup, '7');
        echo "HOTFIX35_LOCALE_SWITCH_SESSION_COOKIE_FALLBACK=PASS locales=ar,en,ar-EG invalid=rejected\n";
        echo "HOTFIX35_RUNTIME_MODE=FIXTURE_ONLY\n";
        echo "HOTFIX35_RUNTIME_VERIFICATION=PASS mode=fixture\n";
        exit(0);
    }
    ...
    try {
        ...
        foreach (User::query()->where('status', 'active')->whereNotNull('email_verified_at')->orderByDesc('is_super_admin')->orderBy('id')->limit(100)->get() as $candidate) {
            ...
            $required = ['dashboard_reports.view', 'purchase_orders.view', 'purchase_invoices_supplier_returns.view', 'inventory_stock_card.view', 'pos_sales.view'];
        ...
        $screens = ['/dashboard', '/alerts', '/approvals', '/sales', '/purchasing/orders', '/purchasing/invoices', '/inventory'];
        ...
        $assert($alertsStatus === 200 && str_contains($alertsHtml, 'بالمصري: التنبيهات'), 'Egyptian alert label is not visible.');
        $app->setLocale('ar-EG');
        ...
        echo "HOTFIX35_AUTHENTICATED_AR_EN_AR_EG_SCREENS=PASS count=21 status=200 representative=dashboard,alerts,approvals,sales,purchasing,inventory\n";
        echo "HOTFIX35_EGYPTIAN_GREETING_ALERT_SOURCE_LABELS=PASS source={$sourceLabel}\n";
        ...
        echo "HOTFIX35_INVOICE_SEARCH_KEYBOARD_RUNTIME=PASS\n";
    } finally {
        while ($connection->transactionLevel() > $level) {
            $connection->rollBack();
        }
    }
    echo "HOTFIX35_DATABASE_MUTATION=NONE transaction=rolled_back\n";
    echo "HOTFIX35_RUNTIME_VERIFICATION=PASS mode=activation\n";
} catch (Throwable $exception) {
    fwrite(STDERR, 'HOTFIX35_RUNTIME_VERIFICATION=FAIL '.str_replace(["\r", "\n"], ' ', $exception->getMessage())."\n");
    exit(1);
}
