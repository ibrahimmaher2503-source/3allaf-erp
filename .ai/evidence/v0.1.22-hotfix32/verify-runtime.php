...
$assert(ApplicationVersion::RELEASE === '0.1.22-hotfix32', 'Hotfix32 version contract is not active.');
foreach ([
...
$supplierOnlyLabel = 'استخدام منتجات المورد فقط';
$assert(substr_count($invoiceSource, ':label="__(\'Use supplier products only\')"') === 1, 'Invoice source must declare the supplier-only editor control exactly once.');
$assert((json_decode((string) file_get_contents($root.'/lang/ar.json'), true, 512, JSON_THROW_ON_ERROR)['Use supplier products only'] ?? '') === $supplierOnlyLabel, 'Exact Arabic supplier-only label is missing.');
$app->setLocale('ar');
$invoiceEditorControls = Blade::render(<<<'BLADE'
<div data-purchase-invoice-editor>
    <flux:checkbox wire:model.live="supplierProductsOnly" :label="__('Use supplier products only')" />
    <x-product-line-lookup wire:model="lineItems.0.product_id" :purchasing="true" supplier-id="7" :supplier-only="true" currency-code="EGP" />
    <x-product-line-lookup wire:model="lineItems.1.product_id" :purchasing="true" supplier-id="7" :supplier-only="true" currency-code="EGP" />
</div>
BLADE);
$document = new DOMDocument();
$previousLibxmlErrors = libxml_use_internal_errors(true);
try {
    $assert($document->loadHTML('<?xml encoding="utf-8" ?>'.$invoiceEditorControls), 'Rendered invoice editor controls are not valid HTML.');
    $xpath = new DOMXPath($document);
    $exactLabelText = '//div[@data-purchase-invoice-editor]//text()[normalize-space(.)="'.$supplierOnlyLabel.'"]';
    $lookupLabelText = '//*[@data-product-lookup]//text()[normalize-space(.)="'.$supplierOnlyLabel.'"]';
    $assert($xpath->query($exactLabelText)?->length === 1, 'Rendered invoice editor must expose the exact supplier-only label once beside its control.');
    $assert($xpath->query($lookupLabelText)?->length === 0, 'Product lookup controls must not duplicate the supplier-only label.');
} finally {
    libxml_clear_errors();
...
$runBrowser = static function (string $lookup, string $supplierId) use ($assert, $asset, $node, $root): void {
    ...
    $htmlFile = tempnam(sys_get_temp_dir(), 'hotfix32-browser-');
...
echo "HOTFIX32_STATIC_MARKUP_ASSET=PASS\n";
if ($mode === '--fixture') {
    $runBrowser($lookups['ar'], '7');
    echo "HOTFIX32_RUNTIME_MODE=FIXTURE_ONLY\n";
    echo "HOTFIX32_RUNTIME_VERIFICATION=PASS mode=fixture\n";
    exit(0);
}
...
try {
    ...
    foreach (['ar', 'en'] as $locale) foreach (['/purchasing/invoices', '/pricing/lists/manage', '/purchasing/orders', '/purchasing/orders/create'] as $uri) {
        ...
        echo 'HOTFIX32_AUTHENTICATED_'.strtoupper($locale).'_'.strtoupper(str_replace(['/', '-'], ['_', '_'], trim($uri, '/')))."=PASS status=200\n";
    }
    ...
    echo "HOTFIX32_AUTHENTICATED_PRODUCT_SEARCH=PASS matches={$matches->count()}\n";
    $runBrowser($lookups['ar'], (string) $association->supplier_id);
} finally {
    while ($connection->transactionLevel() > $level) $connection->rollBack();
}
echo "HOTFIX32_DATABASE_MUTATION=NONE transaction=rolled_back\n";
echo "HOTFIX32_RUNTIME_VERIFICATION=PASS mode=activation\n";
} catch (Throwable $exception) {
    fwrite(STDERR, 'HOTFIX32_RUNTIME_VERIFICATION=FAIL '.str_replace(["\r", "\n"], ' ', $exception->getMessage())."\n");
    exit(1);
}
