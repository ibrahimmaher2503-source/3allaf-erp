...
final class RecordSupplierPaymentAction
{
    ...
    public function execute(User $actor, Supplier $supplier, PaymentMethod $method, string $amount, array $allocations, string $idempotencyKey, ?string $paymentDate = null, string $currencyCode = 'EGP', ?int $cashAccountId = null, ?string $reference = null, ?string $evidenceReference = null, ?string $notes = null): SupplierPayment
    {
        ...
        if ($idempotencyKey === '') throw new InvalidArgumentException(__('A supplier payment idempotency key is required.'));
        ...
        if ($allocations === [] || count(array_unique(array_column($allocations, 'purchase_invoice_id'))) !== count($allocations)) throw new InvalidArgumentException(__('Supplier payment allocations must reference distinct invoices.'));
        if (bccomp(array_reduce($allocations, fn (string $total, array $allocation): string => bcadd($total, $allocation['amount'], 4), '0.0000'), $amount, 4) !== 0) throw new InvalidArgumentException(__('Supplier payment allocations must equal the payment amount.'));
        ...
        try {
            return DB::transaction(function () use ($actor, $supplier, $method, $amount, $allocations, $idempotencyKey, $paymentDate, $currencyCode, $cashAccountId, $reference, $evidenceReference, $notes, $payloadHash): SupplierPayment {
                ...
                if ($invoices->count() !== count($invoiceIds)) throw new InvalidArgumentException(__('Every supplier payment allocation must reference an existing invoice.'));
                ...
                if ($supplier->status !== 'active') throw new InvalidArgumentException(__('Payments can only be recorded for an active supplier.'));
                ...
                foreach ($allocations as $allocation) {
                    ...
                    if ((int) $invoice->supplier_id !== (int) $supplier->id || $invoice->status !== 'approved') throw new InvalidArgumentException(__('Supplier payments may allocate only to approved invoices for the same supplier.'));
                    if (strtoupper((string) ($invoice->currency_code ?: 'EGP')) !== $currencyCode) throw new InvalidArgumentException(__('Supplier payment currency must match every allocated invoice.'));
                    if (bccomp($allocation['amount'], app(SupplierBalance::class)->outstandingForInvoice($invoice), 4) > 0) throw new InvalidArgumentException(__('A supplier payment allocation exceeds the invoice outstanding amount.'));
                }
                $cashAccount = null;
                if ($cashAccountId !== null) {
                    $companyIds = Store::query()->whereIn('id', $invoices->pluck('store_id'))->pluck('company_id')->filter()->unique();
                    if ($companyIds->count() !== 1) throw new InvalidArgumentException(__('Supplier payment allocations must belong to one company when a cash account is selected.'));
                    $cashAccount = CashAccount::query()->where('company_id', $companyIds->first())->where('currency_code', $currencyCode)->where('status', 'active')->lockForUpdate()->findOrFail($cashAccountId);
                }
    ...
    {
        if (! hash_equals((string) $payment->payload_hash, $payloadHash)) throw new InvalidArgumentException(__('This supplier payment idempotency key was already used with different data.'));
    ...
    {
        $value = trim($value);
        if (! preg_match('/^(?:0|[1-9]\d*)(?:\.\d{1,4})?$/', $value) || bccomp($value, '0', 4) <= 0) throw new InvalidArgumentException(__('Supplier payment amount must be a positive decimal.'));
    ...
    private function currency(string $value): string
    {
        ...
        if (! preg_match('/^[A-Z]{3}$/', $value)) throw new InvalidArgumentException(__('Currency code must contain three letters.'));
        return $value;
    }
    ...
    private function date(string $value): string
    {
        ...
        if ($parts === [] || ! checkdate((int) $matches[2], (int) $matches[3], (int) $matches[1])) throw new InvalidArgumentException(__('Supplier payment date is invalid.'));
        return $value;
    }
}
