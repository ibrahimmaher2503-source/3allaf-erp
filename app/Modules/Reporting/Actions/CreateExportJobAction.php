...
final class CreateExportJobAction
{
    ...
    public function execute(User $user, array $filters, string $format = 'xlsx'): ExportJob
    {
        ...
        $snapshot = $dataset ? app(CentralExportSnapshot::class)->execute($user, $dataset, $filters) : app(ReportSnapshot::class)->execute($user, $filters, true);
        $fingerprint = $dataset ? app(CentralExportSnapshot::class)->fingerprint($snapshot) : app(ReportSnapshot::class)->fingerprint($snapshot);
        ...
        if ($created) app(RecordAuditEvent::class)->execute(
            category: 'reporting',
        ...
        if ($created) GenerateReportExportJob::dispatch($job->id)->onQueue('reports');
